...
namespace HHHBBBBTT
{
  ...
  StatusCode TriggerSFAlg::initialize()
  {
    ...
    ATH_CHECK (m_bbbbttEleHandle.initialize(m_systematicsList));
    ...
    ATH_CHECK (m_selected_el.initialize(m_systematicsList, m_bbbbttEleHandle));
  ...
  StatusCode TriggerSFAlg::execute()
  {
    ...
    for (const auto& sys : m_systematicsList.systematicsVector())
    {
      ...
      ANA_CHECK (m_bbbbttEleHandle.retrieve (electrons, sys));
  ...
  (const xAOD::EventInfo *event, const CP::SystematicSet& sys,
   ...
   const xAOD::TauJet* tau0, const xAOD::TauJet* tau1){
    ...
    if(m_categoryBranches.at(HHHBBBBTT::pass_baseline_SLT).get(*event, sys) ||
       m_categoryBranches.at(HHHBBBBTT::pass_ZCR).get(*event, sys) ||
       m_categoryBranches.at(HHHBBBBTT::pass_TopEMuCR).get(*event, sys)){
      if(ele0){
    ...
    else if(m_categoryBranches.at(HHHBBBBTT::pass_baseline_LTT).get(*event, sys)){
      if(ele0){
    ...
    else if(m_categoryBranches.at(HHHBBBBTT::pass_baseline_STT).get(*event, sys)){
    ...
    else if(m_categoryBranches.at(HHHBBBBTT::pass_baseline_DTT).get(*event, sys)){
