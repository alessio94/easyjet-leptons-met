...
namespace ZQG
{
  BaselineVarsZQGAlg::BaselineVarsZQGAlg(const std::string &name,
  ...
  StatusCode BaselineVarsZQGAlg::initialize()
  {
    // Read syst-aware input handles
    ATH_CHECK (m_ZQGJetHandle.initialize(m_systematicsList));
    ATH_CHECK (m_ZQGLRJetHandle.initialize(m_systematicsList));
    ATH_CHECK (m_ZQGElectronHandle.initialize(m_systematicsList));
    ATH_CHECK (m_ZQGMuonHandle.initialize(m_systematicsList));
    ATH_CHECK (m_trackHandle.initialize(m_systematicsList));
    ATH_CHECK (m_eventHandle.initialize(m_systematicsList));

    if(m_isMC){
      ATH_CHECK (m_ele_truthOrigin.initialize(m_systematicsList, m_ZQGElectronHandle));
      ATH_CHECK (m_ele_truthType.initialize(m_systematicsList, m_ZQGElectronHandle));
      ATH_CHECK (m_mu_truthOrigin.initialize(m_systematicsList, m_ZQGMuonHandle));
      ATH_CHECK (m_mu_truthType.initialize(m_systematicsList, m_ZQGMuonHandle));
    ...
    if (!m_isBtag.empty()) {
      ATH_CHECK (m_isBtag.initialize(m_systematicsList, m_ZQGJetHandle));
    }
    ...
    for (const std::string &var : m_PCBTnames){
      ...
      ATH_CHECK(m_PCBTs.at(var).initialize(m_systematicsList, m_ZQGJetHandle));
    };


    if (m_isMC) {
      ATH_CHECK (m_truthFlav.initialize(m_systematicsList, m_ZQGJetHandle));
    }
    ...
    ATH_CHECK (m_GN2Xv01_phbb.initialize(m_systematicsList, m_ZQGLRJetHandle));
    ATH_CHECK (m_GN2Xv01_phcc.initialize(m_systematicsList, m_ZQGLRJetHandle));
    ATH_CHECK (m_GN2Xv01_pqcd.initialize(m_systematicsList, m_ZQGLRJetHandle));
    ATH_CHECK (m_GN2Xv01_ptop.initialize(m_systematicsList, m_ZQGLRJetHandle));
  ...
  StatusCode BaselineVarsZQGAlg::execute()
  {
    ...
    for (const auto& sys : m_systematicsList.systematicsVector())
    {
      ...
      ANA_CHECK (m_ZQGJetHandle.retrieve (jets, sys));

      const xAOD::JetContainer *largeJets  = nullptr;
      ANA_CHECK (m_ZQGLRJetHandle.retrieve (largeJets , sys));

      const xAOD::MuonContainer *muons = nullptr;
      ANA_CHECK (m_ZQGMuonHandle.retrieve (muons, sys));

      const xAOD::ElectronContainer *electrons = nullptr;
      ANA_CHECK (m_ZQGElectronHandle.retrieve (electrons, sys));
      ...
      // l-jet sector
      std::vector<int> untag_values = {0};
      auto ljets = std::make_unique<ConstDataVector<xAOD::JetContainer>> (SG::VIEW_ELEMENTS);
      if(std::find(m_PCBTnames.begin(), m_PCBTnames.end(), ftag2D_WP)!=m_PCBTnames.end())
      {
        for(const xAOD::Jet* jet : *jets) {
          int pcbt = m_PCBTs.at(ftag2D_WP).get(*jet, sys);
          if (std::find(untag_values.begin(), untag_values.end(), pcbt)!=untag_values.end() && std::abs(jet->eta())<2.5) {
            ljets->push_back(jet);
          }
        }
      }
      int n_ljets = ljets->size();
      ...
      m_Ibranches.at("nLJets").set(*event, n_ljets, sys);
      ...
      if (n_bjets >=2){
        int bjet0_val = m_PCBTs.at(ftag2D_WP).get(*bjets->at(0), sys);
        int bjet1_val = m_PCBTs.at(ftag2D_WP).get(*bjets->at(1), sys);
        int PCFT2D2bjets = 0;
  
        for (int i = 4; i <= 6; i++) {
          for (int j = 1; j <= i; j++) {  // Only consider the case where j <= i.
              PCFT2D2bjets++;
              if ((bjet0_val == i && bjet1_val == j) || (bjet0_val == j && bjet1_val == i)) {
                  m_Ibranches.at("PCFT2D2bjets").set(*event, PCFT2D2bjets, sys);
              }
          }
        }
          ...
          for (int i = 1; i <= 3; i++) {
            for (int j = 1; j <= i; j++) {  // Only consider the case where j <= i.
                ...
                if ((cjet0_val == i && cjet1_val == j) || (cjet0_val == j && cjet1_val == i)) {
                    m_Ibranches.at("PCFT2D2cjets").set(*event, PCFT2D2cjets, sys);
                }
            }
        }
      ...
      //L-jet sector
      if(nLeptons >= 2 && n_ljets){
        m_Fbranches.at("dRZL").set(*event, ll.DeltaR(ljets->at(0)->p4()), sys);
        m_Fbranches.at("dPhiZL").set(*event, ll.DeltaPhi(ljets->at(0)->p4()), sys);
      }

      for (int i=0; i<std::min(n_ljets, 2); i++){
        std::string prefix = "Jet_l"+std::to_string(i+1);
        m_Fbranches.at(prefix+"_pt").set(*event, ljets->at(i)->pt(), sys);
        m_Fbranches.at(prefix+"_eta").set(*event, ljets->at(i)->eta(), sys);
        m_Fbranches.at(prefix+"_phi").set(*event, ljets->at(i)->phi(), sys);
        m_Fbranches.at(prefix+"_E").set(*event, ljets->at(i)->e(), sys);
        if (m_isMC) {
          m_Ibranches.at(prefix+"_truthLabel").set(*event, m_truthFlav.get(*ljets->at(i), sys), sys);
        }
      }

      if (n_ljets >=2){  
        TLorentzVector LL = ljets->at(0)->p4()+ljets->at(1)->p4();
        m_Fbranches.at("mLL").set(*event, LL.M(), sys);
        m_Fbranches.at("pT_over_mLL").set(*event, (LL.Pt())/(LL.M()), sys);
        m_Fbranches.at("pTLL").set(*event, LL.Pt(), sys);
        m_Fbranches.at("EtaLL").set(*event, LL.Eta(), sys);
        m_Fbranches.at("PhiLL").set(*event, LL.Phi(), sys);
        m_Fbranches.at("dRLL").set(*event, (ljets->at(0)->p4()).DeltaR(ljets->at(1)->p4()), sys);
        m_Fbranches.at("dPhiLL").set(*event, (ljets->at(0)->p4()).DeltaPhi(ljets->at(1)->p4()), sys);
        m_Fbranches.at("dEtaLL").set(*event, (ljets->at(0)->eta()) - ljets->at(1)->eta(), sys);
      }
