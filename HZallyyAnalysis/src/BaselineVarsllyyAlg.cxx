...
namespace HZALLYY {
  ...
  StatusCode BaselineVarsllyyAlg::initialize() {
    ...
    for (const std::string &
        var: m_floatVariables) {
      CP::SysWriteDecorHandle < float > whandle {
        var +"_%SYS%", this
      };
    ...
    for (const std::string &
        var: m_intVariables) {
      ATH_MSG_DEBUG("initializing integer variable: " <<
        var);
      CP::SysWriteDecorHandle < int > whandle {
        var +"_%SYS%", this
      };
  ...
  StatusCode BaselineVarsllyyAlg::execute() {
    ...
    for (const auto & sys: m_systematicsList.systematicsVector()) {
      ...
      static
      const SG::AuxElement::ConstAccessor < int > HadronConeExclTruthLabelID("HadronConeExclTruthLabelID");
	...
	if(m_isMC){
      ...
      std::vector < std::pair <
        const xAOD::IParticle * , int >> leptons;
      ...
      std::sort(leptons.begin(), leptons.end(),
        [](const std::pair <
          const xAOD::IParticle * , int > & a,
            const std::pair <
              const xAOD::IParticle * , int > & b) {
