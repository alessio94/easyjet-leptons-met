...
namespace VBSHIGGS{
  ...
  StatusCode SemiLepSelectorAlg::execute(){
    ...
    for (const auto& sys : m_systematicsList.systematicsVector()){
      ...
      if (m_passTriggerSLT.get(*event, sys)) {
  ...
  void SemiLepSelectorAlg :: leptonSelection (const xAOD::ElectronContainer& electrons, const xAOD::MuonContainer& muons, const xAOD::MissingET *met){
    ...
    if (electrons.size() + muons.size() == 1) m_bools.at(VBSHIGGS::PASS_ONE_LEPTON) = true;
  ...
  //Resolved Analysis
  void SemiLepSelectorAlg :: resolvedSelection(const xAOD::JetContainer *HJets, const std::vector<const xAOD::Jet*>& bjets, const CP::SystematicSet& sys){
    ...
    bool pass_resolved_baseline = m_bools.at(VBSHIGGS::PASS_TRIGGER) && m_bools.at(VBSHIGGS::PASS_ONE_LEPTON) && m_bools.at(VBSHIGGS::PASS_RES_AT_LEAST_ONE_B_JET );
  ...
  //boosted Analysis
  void SemiLepSelectorAlg :: boostedSelection(const xAOD::JetContainer *largeJets, const CP::SystematicSet& sys){
    ...
    bool pass_merged_baseline = m_bools.at(VBSHIGGS::PASS_TRIGGER) && m_bools.at(VBSHIGGS::PASS_ONE_LEPTON) && XbbScore > 1.560 ;
